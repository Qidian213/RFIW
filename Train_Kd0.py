...
KD_ID = 0
os.environ['CUDA_VISIBLE_DEVICES'] = f'{KD_ID}'
...
### dataset
    kd_id  = KD_ID
    kd_num = 7
    data_folders_path = "/data/zzg/FGRFIW/Images/"
...
###Model
    ...
    save_file_path = "Kd" +str(kd_id)+ "/p1_best.h5"

    checkpoint        = ModelCheckpoint(save_file_path, monitor='val_acc', verbose=1, save_best_only=True, mode='max', period=1)
    reduce_on_plateau = ReduceLROnPlateau(monitor="val_acc", mode="max", factor=0.1, patience=15, verbose=1)
    callbacks_list    = [checkpoint, reduce_on_plateau]

    Rfiw_model = RFIW_Model() 
